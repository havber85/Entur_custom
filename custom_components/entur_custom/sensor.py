...
from homeassistant.custom_components.entur_custom.sensor import PLATFORM_SCHEMA, SensorEntity
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        vol.Required(CONF_STOP_IDS): vol.All(
            vol.Schema(
                {
...
async def async_setup_platform(
    ...
    for stop_config in config[CONF_STOP_IDS]:
        stop_id = stop_config["stop_id"]
        line_whitelist = stop_config.get(CONF_WHITELIST_LINES, [])

        stops = [s for s in [stop_id] if "StopPlace" in s]
        quays = [s for s in [stop_id] if "Quay" in s]
        ...
        try:
            given_name = f"{name} {data.get_stop_info(stop_id).name}"
        except KeyError:
            given_name = f"{name} {stop_id}"

        entities.append(
            EnturPublicTransportSensor(proxy, given_name, stop_id, show_on_map)
        )
